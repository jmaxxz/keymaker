...
async function onSecResponse(session, data) {
  var secCmd = new SecCommand(data)
  if(secCmd.command == 0x18){
    var lastIndex = secCmd.rawBuffer.readUInt8(4);
    if(lastIndex < 255) {
      return await session.secWrite(cmd.clearKeySlot(lastIndex + 1).data);
    }
  }
  session.disconnect();
}
